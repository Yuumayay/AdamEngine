...
func category():
	for i in list.get_children():
		list.remove_child(i)
	var ind := 0
	var json = Setting.adv_setting.category
	for i in json:
...
func option():
	...
	if list.get_child_count() == 0:
		...
		if "data" in list.get_child(selectList[layer]):
			...
			selected_name = selecting
	var json = Setting.adv_setting.category[selected_name]
	for i in list.get_children():
		list.remove_child(i)
	for i in json:
		var new_item
		...
		var itemname = i
		var current = Setting.adv_setting.category[selected_name][i]["cur"]
		var itemtype = Setting.adv_setting.category[selected_name][i]["type"]
		...
		match itemtype:
			"engineType":
				if layer == 0 or layer == 1:
					for index in Setting.adv_setting.category[selected_name][i]["metadata"]:
						for indexind in Setting.adv_setting.category[selected_name][i]["array"]:
					...
					var index := 0
					for type in Setting.adv_setting.category[selected_name][i]["array"]:
						if !Setting.eng():
					...
					var index := 0
					var type = Setting.adv_setting.category[selected_name][i]["array"][select3]
			...
			"bind":
				...
				var key = Setting.adv_setting.category[selected_name][i]["key"]
			...
			"array":
				if !Setting.eng():
					valuetext.text = Setting.translate(Setting.adv_setting.category[selected_name][i]["array"][current])
				else:
					valuetext.text = Setting.adv_setting.category[selected_name][i]["array"][current]
			"bool":
				if current:
					if !Setting.eng():
